...
private:
    ...
    std_msgs::Float32 motorCmdVelmsg[3];
...
BaseController::BaseController(void)
{
    ...
    _nh.param("motor_vel_coeff", this->VelocityCoefficient, 15.873015873);	//radian per metre for the wheels, which is equivalent to the inverse of the wheel radius
    _nh.param("robot_radius", this->RobotRadius, 0.15325);
    _nh.param("wheel_radius", this->wheel_radius, 0.063);
    ...
    motor0CmdVel_pub = nh.advertise<std_msgs::Float32>("motor0_cmd_vel", 1);
